...
$fetch = mysql_query("SELECT * FROM mst_subject LIMIT $startrow, 10")or
die(mysql_error());
   $num=mysql_num_rows($fetch);
        if($num>0)
        {
			echo "<p class='head1' style='margin-top:50px; margin-bottom:50px; margin-left:120px;'>Update Subject </p>"; ?>
<?php /* ?><div style='margin:auto;width:90%;height:300px;box-shadow:2px 1px 2px 2px #CCCCCC;text-align:center; margin-top:50px; margin-bottom:50px;'>";<?php */ ?> <?php
        ...
        echo "<center><table class='table-striped' border='4'>";
        echo "<tr class=style11><td>sub_name</td></tr>";
        for($i=0;$i<$num;$i++)
        {
        $row=mysql_fetch_row($fetch);
        echo "<tr>";
		"<td class=style4>$row[0]</td>";
        echo"<td class=style4>$row[1]</td>";?>
        <td><a href="editsub.php?sub_id=<?php echo $row['0'];?>"><input type="button" name="Edit" value="Edit"></td>
      <td><a href="deletesub.php?sub_id=<?php echo $row['0'];?>"></a><input type="button" name="Delete" value="Delete" onClick="return confirmDelete()"></td>
...
if ($prev >= 0)
    echo '<a class=style2 href="'.$_SERVER['PHP_SELF'].'?startrow='.$prev.'">Previous</a></center>';
?>
</form>
